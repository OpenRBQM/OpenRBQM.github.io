...
# home page). Each card's heading links to the GitHub source; when the repo has
# a homepage (pkgdown website), a "Website" link is added below the description.
render_cards <- function(df) {
  df <- df %>%
    mutate(
      website_link = ifelse(
        !is.na(homepage) & nzchar(homepage),
        glue("[Website]({homepage})"),
        ""
      )
    )
  cat("\n::: {.grid}\n\n")
...
get_pkgdown_url <- function(org, repo) {
  url <- paste0("https://api.github.com/repos/",org,"/",repo)
    ...
    # Normalize to a trailing slash so "<homepage>articles/..." links are valid.
    if (!is.null(homepage) && nzchar(homepage) && !grepl("/$", homepage)) {
      homepage <- paste0(homepage, "/")
    }
    return(homepage)
}
```
...
- [Cookbook - Adverse Event KRI Report](https://gilead-biostats.github.io/gsm.kri/examples/Cookbook_AdverseEventKRI.html)
- [Cookbook - Adverse Event Workflow Examples](https://gilead-biostats.github.io/gsm.kri/examples/Cookbook_AdverseEventWorkflow.html)
- [Cookbook - Reporting Workflow Examples](https://gilead-biostats.github.io/gsm.kri/examples/Cookbook_ReportingWorkflow.html)

[^gsmkri-examples]: Examples from the [{gsm.kri}](https://gilead-biostats.github.io/gsm.kri/) package documentation.